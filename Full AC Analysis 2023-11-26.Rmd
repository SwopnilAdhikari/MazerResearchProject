...
AC_Data_2022 <- read_csv("../Data/AC_Data_2022.csv")
...
# Transform FFD and LFD to factor variables (no, we won't do that)
...
```{r}
# Mean-centering the data frame for Transects 1-9
...
# Create a single data frame that contains ALL INDIVIDUAL PLANT values from ALL TRANSECTS, including the MEAN-CENTERED VALUES of our variables

# Observe frequency distributions to check whether the data are reasonable (Does anything glaring pop out at you?)

```{r}

AC_MeanCentered_AllTransects <- rbind(ACTransect1, ACTransect2, ACTransect3, ACTransect4, ACTransect5, ACTransect6, ACTransect7, ACTransect8, ACTransect9)

str(AC_MeanCentered_AllTransects)

names(AC_MeanCentered_AllTransects)

ggplot(AC_MeanCentered_AllTransects, aes(FFD)) +
  geom_histogram(bins=15) +
  xlab("First Flowering Date (Day of Year)") +
  ggtitle("First Flowering Date")

ggplot(AC_MeanCentered_AllTransects, aes(Stem_Biomass)) +
  geom_histogram(bins=15) +
  xlab("Stem_Biomass") +
  ggtitle("Stem Biomass (mg)")

ggplot(AC_MeanCentered_AllTransects, aes(Log_Stem_Biomass)) +
  geom_histogram(bins=15) +
  xlab("Log(Stem_Biomass)") +
  ggtitle("Stem Biomass (mg)")

ggplot(AC_MeanCentered_AllTransects, aes(Corolla_Diameter)) +
  geom_histogram(bins=15) +
  xlab("Corolla Diameter (mm)") +
  ggtitle("Corolla Diameter")

ggplot(AC_MeanCentered_AllTransects, aes(Corolla_Area)) +
  geom_histogram(bins=15) +
  xlab("Corolla Area (mm^2)") +
  ggtitle("Corolla Area")

ggplot(AC_MeanCentered_AllTransects, aes(Log_Leaf_Area_mm2)) +
  geom_histogram(bins=15) +
  xlab("Log(Leaf Area (mm^2))") +
  ggtitle("Log(Leaf Area)")


ggplot(AC_MeanCentered_AllTransects, aes(Leaf_Area_mm2)) +
  geom_histogram(bins=15) +
  xlab("Leaf Area (mm^2)") +
  ggtitle("Leaf Area")


ggplot(AC_MeanCentered_AllTransects, aes(Log_Total_Fruits)) +
  geom_histogram(bins=15) +
  xlab("Log(Total_Fruits)") +
  ggtitle("Log(Total_Fruits)")


ggplot(AC_MeanCentered_AllTransects, aes(Log_Total_Fruits_MC)) +
  geom_histogram(bins=15) +
  xlab("Mean-Centered Log(Total_Fruits)") +
  ggtitle("Mean-Centered Log(Total_Fruits)")

ggplot(AC_MeanCentered_AllTransects, aes(Mean_Ind_Seed_Mass_mg)) +
  geom_histogram(bins=15) +
  xlab("Mean Individual Seed Mass (mg)") +
  ggtitle("Mean Individual Seed Mass")

ggplot(AC_MeanCentered_AllTransects, aes(Mean_Ind_Seed_Mass_mg_MC)) +
  geom_histogram(bins=15) +
  xlab("Mean-Centered Mean Individual Seed Mass (mg)") +
  ggtitle("Mean-Centered Mean Individual Seed Mass")


ggplot(AC_MeanCentered_AllTransects, aes(Log_Lifetime_Fecundity)) +
  geom_histogram(bins=15) +
  xlab("Log(Lifetime Fecundity)") +
  ggtitle("Log(Lifetime Fecundity)")

ggplot(AC_MeanCentered_AllTransects, aes(Log_Lifetime_Fecundity_MC)) +
  geom_histogram(bins=15) +
  xlab("Mean-Centered Log(Lifetime_Fecundity)") +
  ggtitle("Mean-Centered Log(Lifetime_Fecundity)")
...
# First, create a vector that contains the names of all of the recipients in all of the transects.
...
# Conduct a mixed model using all of the individual plant data (319 observations) in the new dataframe, called AC_MeanCentered_AllTransects


```{r}

# Donor:Recipient means that Recipients are nested within donors

Model_1a <- lmer(Log_Stem_Biomass_MC ~ 1 + (1|Donor:Recipient), data=AC_MeanCentered_AllTransects)
...
Model_1b <- lmer(Log_Stem_Biomass_MC ~ 1 + (1|Donor) + (1|Donor:Recipient), data=AC_MeanCentered_AllTransects)
...
Model_2 <- lmer(Log_Stem_Biomass_MC ~ 1 + (1|Donor), data=AC_MeanCentered_AllTransects)
...
Model_3 <- lmer(Log_Stem_Biomass_MC ~ 1 + (1|Recipient), data=AC_MeanCentered_AllTransects)
...
Model_4 <- lmer(Log_Stem_Biomass_MC ~ 1 + (1|Recipient) + (1|Donor), data=AC_MeanCentered_AllTransects)
...
Model_5 <- lmer(Log_Lifetime_Fecundity_MC ~ Corolla_Diameter_MC + Log_Stem_Biomass_MC + (1|Donor) + (1|Donor:Recipient), data = AC_MeanCentered_AllTransects)
...
ggplot(AC_MeanCentered_AllTransects, aes(y=Log_Stem_Biomass_MC, x = Corolla_Diameter_MC)) +
  geom_point() +
  xlab("Mean-Centered Corolla Diameter (mm)") +
  ylab("Mean-Centered Log(Stem_Biomass)") +
  theme_classic(base_size=15) +
  stat_smooth(method = "lm", formula = 'y ~ x', se=F, fullrange = T) +
  facet_wrap(~Donor)

ggplot(AC_MeanCentered_AllTransects, aes(Donor, Log_Stem_Biomass_MC, fill=Donor, color=Donor)) +
  geom_violin() +
  theme_bw(base_size=12)

AC_MeanCentered_AllTransects$Transect <- as.factor(AC_MeanCentered_AllTransects$Transect)

ggplot(AC_MeanCentered_AllTransects, aes(Transect, Log_Stem_Biomass, fill=Transect, color=Transect)) +
  geom_violin() +
  theme_bw(base_size=12)

ggplot(AC_MeanCentered_AllTransects, aes(Transect, Corolla_Diameter, fill=Transect, color=Transect)) +
  geom_violin() +
  theme_bw(base_size=12)

ggplot(AC_MeanCentered_AllTransects, aes(Transect, Log_Lifetime_Fecundity, fill=Transect, color=Transect)) +
  geom_violin() +
  theme_bw(base_size=12)

ggplot(AC_MeanCentered_AllTransects, aes(Transect, Mean_Ind_Seed_Mass_mg, fill=Transect, color=Transect)) +
  geom_violin() +
  theme_bw(base_size=12)



# The following 3 models (6a, 6b, and 6c) generate identical results

Model_6a <- lmer(Log_Lifetime_Fecundity_MC ~ (1|Donor) + (1|Donor:Recipient), data = AC_MeanCentered_AllTransects)

Model_6b <- lmer(Log_Lifetime_Fecundity_MC ~ 1 + (1|Donor) + (1|Donor:Recipient), data = AC_MeanCentered_AllTransects)

Model_6c <- lmer(Log_Lifetime_Fecundity_MC ~ 1 + (1|Donor) + (1|Recipient), data = AC_MeanCentered_AllTransects)


summary(Model_6a) #lme4
summary(Model_6b) #lme4
summary(Model_6c) #lme4

ranef(Model_6a) #lme4
ranef(Model_6b) #lme4
ranef(Model_6c) #lme4

summ(Model_6a) #jtools
summ(Model_6b) #jtools
summ(Model_6c) #jtools

ranova(Model_6a) #lmerTest
ranova(Model_6b) #lmerTest
ranova(Model_6c) #lmerTest


Model_7a <- lmer(Corolla_Area_MC ~ (1|Donor) + (1|Donor:Recipient), data = AC_MeanCentered_AllTransects)

Model_7b <- lmer(Corolla_Area_MC ~ 1 + (1|Donor) + (1|Donor:Recipient), data = AC_MeanCentered_AllTransects)

Model_7c <- lmer(Corolla_Area_MC ~ 1 + (1|Donor) + (1|Recipient), data = AC_MeanCentered_AllTransects)


summary(Model_7a) #lme4
summary(Model_7b) #lme4
summary(Model_7c) #lme4

ranef(Model_7a) #lme4
ranef(Model_7b) #lme4
ranef(Model_7c) #lme4

summ(Model_7a) #jtools
summ(Model_7b) #jtools
summ(Model_7c) #jtools

ranova(Model_7a) #lmerTest
ranova(Model_7b) #lmerTest
ranova(Model_7c) #lmerTest

Model_0 <- lmer(Log_Lifetime_Fecundity ~ Corolla_Diameter + (1|Donor) + (1|Donor:Recipient), AC_MeanCentered_AllTransects) # This is mean-centered by transect, so the analysis of this data set shouldn't include Transect as a predictor variable because the effects of transect on all traits is already controlled for.

Model_A <- lmer(Log_Lifetime_Fecundity ~ Corolla_Diameter + (1|Transect) + (1|Donor) + (1|Donor:Recipient), AC_Data_2022)

Model_B <- lmer(Log_Lifetime_Fecundity ~ Corolla_Diameter + (1|Transect) + (1|Donor) + (1|Recipient), AC_Data_2022)

Model_C <- lmer(Log_Lifetime_Fecundity ~ Corolla_Diameter + (1|Transect), AC_Data_2022)

Model_D <- lm(Log_Lifetime_Fecundity ~ Corolla_Diameter, AC_Data_2022)
...
# Estimating Narrow-sense Heritability of Log(Lifetime_Fecundity) controlling for variation in Corolla_Diameter
...
Model_E <- lmer(Log_Lifetime_Fecundity_MC ~ Corolla_Diameter_MC + (1|Donor) + (1|Donor:Recipient), AC_MeanCentered_AllTransects)
...
cor(AC_MeanCentered_AllTransects$Log_Lifetime_Fecundity_MC,AC_MeanCentered_AllTransects$Corolla_Diameter_MC, use="complete.obs") # = 0.09. There is a weak positive correlation between corolla diameter and fecundity, so controlling for corolla diameter will influence (lower) the amount of variation in Fecundity among donors.

# Variance among Donors in Log_Lifetime_Fecundity_MC = 0.09235, controlling for variation among maternal IDs nested within donors AND controlling for variation in Corolla Diameter

# Variance among donors = 0.09235



var(AC_MeanCentered_AllTransects$Log_Lifetime_Fecundity_MC) # Phenotypic variance in Mean-centered Log(Lifetime Fecundity) among INDIVIDUALS = 1.614857

#sd(AC_MeanCentered_AllTransects$Log_Lifetime_Fecundity_MC) # SD in Mean-centered Log(Lifetime Fecundity) among INDIVIDUALS = 1.27

# Narrow-sense Heritability using Model E, controlling for Corolla Diameter = (4 * 0.09235)/1.614857
...
# Now, calculate narrow-sense heritability in Log_Lifetime_Fecundity_MC using a model (Model F) that does NOT account for variation among maternal IDs nested within donors:

Model_F <- lmer(Log_Lifetime_Fecundity_MC ~ Corolla_Diameter_MC + (1|Donor), AC_MeanCentered_AllTransects)
...
# Variance among Donors = 0.1428. This is higher than the variance among donors seen in Model E, which did not control for variation among maternal IDs.

# Narrow-sense heritability = (4 * 0.1428)/1.614857

4 * 0.1428 # = 0.5712

0.5712/1.614857 # = 0.3537, which is higher than the narrow-sense heritability calculated from Model E


# Now, calculate narrow-sense heritability in Log_Lifetime_Fecundity using the dataframe of Donor means, which does not account for variation among maternal IDs nested within donors OR for variation in Lifetime Fecundity due to Corolla_Diameter: 

var(AC_Avg_MC_Population_ByDonor$AMC_Log_Lifetime_Fecundity) # = 0.2905

4 * 0.2905 # = 1.162

1.162/1.614857 # = 0.72.  This is the highest value of heritability so far!

# Let's examine the correlation between Fecundity and Corolla Diameter at three different levels:

# Among paternal family means (n=40)

cor(AC_Avg_MC_Population_ByDonor$AMC_Log_Lifetime_Fecundity, AC_Avg_MC_Population_ByDonor$AMC_Corolla_Diameter) # = 0.38

# Among maternal family means (n=107)

cor(AC_Avg_MC_Population_ByRecip$AMC_Log_Lifetime_Fecundity, AC_Avg_MC_Population_ByRecip$AMC_Corolla_Diameter, use="complete.obs") # = 0.15

# Among all individuals (PHENOTYPIC CORRELATION, n = 319)

cor(AC_MeanCentered_AllTransects$Log_Lifetime_Fecundity_MC,AC_MeanCentered_AllTransects$Corolla_Diameter_MC, use="complete.obs") # = 0.09


AC_MeanCentered_AllTransects$Donor <- as.factor(AC_MeanCentered_AllTransects$Donor)

vioplot(AC_MeanCentered_AllTransects$Log_Lifetime_Fecundity ~ AC_MeanCentered_AllTransects$Donor, col = 2:length(levels(AC_MeanCentered_AllTransects$Donor)), xlab="Donor", ylab = "Log(Fecundity)")

# run the vioplot code above WITH the stripchart code below in order to see all of the points within each donor.  The points will be jittered if there are enough of them.

stripchart(AC_MeanCentered_AllTransects$Log_Lifetime_Fecundity ~ AC_MeanCentered_AllTransects$Donor, vertical = TRUE, method = "jitter", pch = 19, add = TRUE, col = 3:8)

# ********

vioplot(AC_MeanCentered_AllTransects$Log_Stem_Biomass ~ AC_MeanCentered_AllTransects$Donor, col = 2:length(levels(AC_MeanCentered_AllTransects$Donor)), xlab="Donor", ylab = "Log(Stem Biomass)")

stripchart(AC_MeanCentered_AllTransects$Log_Stem_Biomass ~ AC_MeanCentered_AllTransects$Donor, vertical = TRUE, method = "jitter", pch = 19, add = TRUE, col = 3:8)

# ********

vioplot(AC_MeanCentered_AllTransects$Corolla_Diameter ~ AC_MeanCentered_AllTransects$Donor, col = 2:length(levels(AC_MeanCentered_AllTransects$Donor)), xlab="Donor", ylab = "Corolla Diameter (mm)")

stripchart(AC_MeanCentered_AllTransects$Corolla_Diameter ~ AC_MeanCentered_AllTransects$Donor, vertical = TRUE, method = "jitter", pch = 19, add = TRUE, col = 3:8)

# ********
vioplot(AC_MeanCentered_AllTransects$FFD ~ AC_MeanCentered_AllTransects$Donor, col = 2:length(levels(AC_MeanCentered_AllTransects$Donor)), xlab="Donor", ylab = "Corolla Diameter (mm)")

stripchart(AC_MeanCentered_AllTransects$FFD ~ AC_MeanCentered_AllTransects$Donor, vertical = TRUE, method = "jitter", pch = 19, add = TRUE, col = 3:8)

# ********

vioplot(AC_MeanCentered_AllTransects$Flowering_Duration ~ AC_MeanCentered_AllTransects$Donor, col = 2:length(levels(AC_MeanCentered_AllTransects$Donor)), xlab="Donor", ylab = "Corolla Diameter (mm)")

stripchart(AC_MeanCentered_AllTransects$Flowering_Duration ~ AC_MeanCentered_AllTransects$Donor, vertical = TRUE, method = "jitter", pch = 19, add = TRUE, col = 3:8)

# ********

# The violin plots are ordered by default by the order of the levels of the categorical variable (in the example above, the category is Donor, and the violins are ordered by the numer of the Donor family). 

par(mfrow = c(1, 2))

data <- AC_MeanCentered_AllTransects

#----------------
# Lower to higher
#----------------

medians <- reorder(data$Donor, data$Log_Lifetime_Fecundity_MC, median)
# medians <- with(data, reorder(Donor, Lifetime_Fecundity, median)) # Equivalent

vioplot(data$Log_Lifetime_Fecundity_MC ~ medians, col = 2:(length(levels(data$Donor)) + 1),
        xlab = "", ylab = "Mean-Centered Log(Lifetime Fecundity)", las = 2)
...
# We can also use vioplot to show the full distributions of multiple columns
# We'll use the raw values of our measured variables
# These are contained in columns 10 (FFD) through 20 (Flowering_Duration) of AC_MeanCentered_AllTransects

vioplot(AC_MeanCentered_AllTransects$FFD, AC_MeanCentered_AllTransects$LFD, AC_MeanCentered_AllTransects$Flowering_Duration, col = 10:20, border = 10:20)
